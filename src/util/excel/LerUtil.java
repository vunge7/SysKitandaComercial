...
public class LerUtil {
    ...
    private final int POS_COD_BARRA = 8;
    private final int POS_QTD_EXISTENTE = 7;
    ...
    public void ler() {
        ...
        try (BufferedInputStream buf = new BufferedInputStream(new FileInputStream(arquivoExcel))) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) { // Ignora cabeçalho
    ...
    private void importarProduto(DadosUtil dados) {
        try {
            ...
            if (tipo == null) throw new RuntimeException("Categoria padrão 'SEM CATEGORIA' não existe no banco!");
    ...
    private void registrarPreco(int idProduto, BigDecimal precoCompra, BigDecimal precoVenda) {
        if (precoCompra == null || precoVenda == null) return;
