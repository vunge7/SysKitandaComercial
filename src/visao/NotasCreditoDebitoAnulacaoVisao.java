        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    .addComponent(cmbTipoDocumento, 0, 182, Short.MAX_VALUE)
...
//        bancoDao = new BancoDao( emf );
        ...
        cmbTipoDocumento.setModel(
                new DefaultComboBoxModel(
                        documentosController.getVectorById(
    ...
    private void procedimento_anular_nota() throws Exception
    {
    ...
    private void constructCreditNote( TbVenda note, TbVenda localSale ) throws SQLException
    {
        note.setCodigoUsuario( usuarioDao.findTbUsuario( cod_usuario ) );
        note.setTotalVenda( localSale.getTotalVenda() );
        note.setPerformance( "false" );
        note.setCredito( "false" );
        note.setValorEntregue( localSale.getValorEntregue() );
        note.setTroco( localSale.getTroco() );
        note.setIdArmazemFK( localSale.getIdArmazemFK() );
        note.setDataVenda( new Date() );
        note.setHora( new Date() );
        note.setDataVencimento( localSale.getDataVencimento() );
        //Dados do cliente
        note.setNomeCliente( localSale.getNomeCliente() );
        note.setClienteNif( localSale.getClienteNif() );
        note.setCodigoCliente( localSale.getCodigoCliente() );

        note.setDescontoTotal( localSale.getDescontoTotal() );
        note.setTotalGeral( localSale.getTotalGeral() );
//                notas.setIdBanco( venda_local.getIdBanco() );
        note.setStatusEliminado( "true" );
        note.setFkAnoEconomico( this.anoEconomico );
        note.setFkCambio( localSale.getFkCambio() );
        // notas.setFkDocumento( getDocumento() );
        note.setFkDocumento( getDocumento() );
...
//                nota_local.setMotivo( motivoJTextArea.getText() );
        ...
        note.setCodFact( prox_doc );
        note.setRefCodFact( localSale.getCodFact() );
        note.setTotalIva( localSale.getTotalIva() );
...
//                nota_local.setAssinatura( MetodosUtil.assinatura_doc( nota_local.getHashCod() ) );
        ...
        note.setRefDataFact( localSale.getDataVenda() );
        System.out.println( "STATUS:documento assinado com sucesso." );
        note.setDescontoComercial( localSale.getDescontoComercial() );
        note.setDescontoFinanceiro( localSale.getDescontoFinanceiro() );
        note.setTotalIncidencia( localSale.getTotalIncidencia() );
        note.setTotalIncidenciaIsento( localSale.getTotalIncidenciaIsento() );

        note.setStatusEliminado( ANULADO.toString() );
//                nota_local.setAreaVenda( venda_local.getAreaVenda() );
        note.setObs( motivoJTextArea.getText() );
        note.setGorjeta( localSale.getGorjeta() );
        note.setStatusRecibo( false );
        note.setCont( 0 );
        note.setIdBanco( new TbBanco( 1 ) );

    }

    public void anularFaturaNota() throws Exception
    {
        boolean anular = exibirMensagemConfirmacao( this, "Mensagem de confirmação", "Deseja mesmo anular esta factura?" ) == 0;

        if ( anular )
        {
            TbVenda orginDoc = getVenda();
            Date data_actual = new Date();

            Calendar calendarHoje = Calendar.getInstance();
            Calendar calendarVenda = Calendar.getInstance();
            calendarVenda.setTime( orginDoc.getDataVenda() );
            System.err.println( "Venda : " + new SimpleDateFormat( "dd/MMM/yyyy" ).format( orginDoc.getDataVenda() ) );
            ...
            if ( dataDeHojePosteriorADataDaVenda )
            {
                ...
                Documento documento = documentosController.findDocumentoById(
                        getIdDocumento()
                );
                TbCliente cliente = clientesController.findByCodigo(
                        orginDoc.getCodigoCliente().getCodigo()
                );
                TbVenda localNote = new TbVenda();
                constructCreditNote( localNote, orginDoc );
                Log.info( "##builder credit note sucessful" );

                if ( FacturaElectronicaUtil.criarFE(
                        localNote,
                        dadosInstituicao,
                        documento,
                        cliente,
...
//                        Integer last_nota = VendaDao.criarVendaComProcedu( nota_local, conexao );
                        Integer lastNoteId = vendasController.salvarRetornaID( localNote );

                        System.out.println( "STATUS:factura criada com sucesso." );
                        boolean documentosalvoComSucesso = salvarItemNotaAnulamento( lastNoteId, orginDoc );
                        ...
                        if ( documentosalvoComSucesso )
                        {
                            ...
                            alterarStatusVenda( orginDoc );

                            if ( orginDoc.getFkDocumento().getPkDocumento() == DVML.DOC_RECIBO_RC )
                            {
                                //eliminar as armotizações da referência do documento
                                if ( !Objects.isNull( VendaDao.findByCodFact( orginDoc.getRefCodFact() ) ) )
                                {
                                    eliminar_amortizacoes( VendaDao.findByCodFact( orginDoc.getRefCodFact() ).getCodigo() );
                                }
                            ...
                            String telefone = orginDoc.getCodigoCliente().getTelefone();
                            String morada = orginDoc.getCodigoCliente().getMorada();
                            String email = orginDoc.getCodigoCliente().getEmail();

                            List<TbProduto> lista_produto_isentos = new ArrayList<>();
                            lista_produto_isentos = getProdutosIsentos( orginDoc );
...
//                    hashMap.put ( "_CLIENTE_NIF", DVML._NAO_INCLUIR );
                            ...
                            System.out.println( "#Codigo Venda#: " + orginDoc.getCodigo() );
                            pagamentoMensalidadeController.removerItem(
                                    orginDoc.getCodigo(),
                                    itemVendasController.listarTodosByIdVenda( orginDoc.getCodigo() )
                            );

                            //Chama a factura e imprime directamente para a imprissora que estiver definidade no sistema operativo
                            ListaNotaDebito listaNotaDebito = new ListaNotaDebito( lastNoteId, Abreviacao.NC, false, false, hashMap, motivos_isentos );
...
//
    ...
    private static TableColumIdUtil getObjectsColumnsIds()
    {
        ...
        tableColumIdUtil.setCOLUMN_DESIGNACAO( 1 );
        tableColumIdUtil.setCOLUMN_PRECO_UNITARIO( 2 );
        tableColumIdUtil.setCOLUMN_QTD( 3 );
        tableColumIdUtil.setCOLUMN_DESCONTO( 4 );
        tableColumIdUtil.setCOLUMN_TAXA( 5 );
        tableColumIdUtil.setCOLUMN_RETENCAO( 6 );
        tableColumIdUtil.setCOLUMN_SUBTOTAL( 7 );
        return tableColumIdUtil;
    }
    ...
    private void visualizarSeries()
    {
        ...
        List<Series> listarPorDocumentoEAno = seriesController.listarPorDocumentoEAno(
                idDocumento,
                idAnoEconomico
        );
        ...
        listarPorDocumentoEAno
                .stream()
                .forEach( e -> cmbSeries.addItem( e.getDesignacao() ) );
