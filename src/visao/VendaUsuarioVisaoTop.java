 ...
 */
public class VendaUsuarioVisaoTop extends javax.swing.JFrame
{
    ...
    public VendaUsuarioVisaoTop( int cod_usuario, BDConexao conexao ) throws SQLException
    {

        initComponents();
        metodo_render_tabela();
//        captura_tecla_tabela();
        confiLabel();
        setLocationRelativeTo( null );
        this.setExtendedState( VendaUsuarioVisaoTop.MAXIMIZED_BOTH );
        jLabel7.setVisible( false );
        txtKilometragem.setVisible( false );

        //pegarResolucao();
        VendaUsuarioVisaoTop.conexao = conexao;

        /**
         * INSTANCIAS DOS CONTROLLER COMERCIAL
         */
        vendasController = new VendasController( VendaUsuarioVisaoTop.conexao );
        itemVendasController = new ItemVendasController( VendaUsuarioVisaoTop.conexao );
        mesasController = new MesasController( VendaUsuarioVisaoTop.conexao );
        lugaresController = new LugaresController( VendaUsuarioVisaoTop.conexao );
        produtosController = new ProdutosController( VendaUsuarioVisaoTop.conexao );
        stocksController = new StoksController( VendaUsuarioVisaoTop.conexao );
        precosController = new PrecosController( VendaUsuarioVisaoTop.conexao );
        tipoProdutoController = new TipoProdutosController( VendaUsuarioVisaoTop.conexao );
        familiaController = new FamiliasController( VendaUsuarioVisaoTop.conexao );
        armazensController = new ArmazensController( VendaUsuarioVisaoTop.conexao );
        localController = new LocalController( VendaUsuarioVisaoTop.conexao );
        unidadesController = new UnidadesController( VendaUsuarioVisaoTop.conexao );
        anoEconomicoController = new AnoEconomicoController( VendaUsuarioVisaoTop.conexao );
        clientesController = new ClientesController( VendaUsuarioVisaoTop.conexao );
        documentosController = new DocumentosController( VendaUsuarioVisaoTop.conexao );
        moedasController = new MoedasController( VendaUsuarioVisaoTop.conexao );
        descontosController = new DescontosController( VendaUsuarioVisaoTop.conexao );
        usuariosController = new UsuariosController( VendaUsuarioVisaoTop.conexao );
        dadosInstituicaoController = new DadosInstituicaoController( VendaUsuarioVisaoTop.conexao );
        produtosImpostoController = new ProdutosImpostoController( VendaUsuarioVisaoTop.conexao );
        produtosIsentoController = new ProdutosIsentoController( VendaUsuarioVisaoTop.conexao );
        caixasController = new CaixasController( VendaUsuarioVisaoTop.conexao );
        formaPagamentoController = new FormaPagamentoController( VendaUsuarioVisaoTop.conexao );
        armazensAccessoController = new ArmazensAccessoController( VendaUsuarioVisaoTop.conexao );
        cambiosController = new CambiosController( VendaUsuarioVisaoTop.conexao );
        formaPagamentoItemController = new FormaPagamentoItemController( VendaUsuarioVisaoTop.conexao );
        servicosRetencaoController = new ServicosRetencaoController( VendaUsuarioVisaoTop.conexao );
        contaController = new ContaController( VendaUsuarioVisaoTop.conexao );
                ...
                try
                {
                    if ( validar() )
                    {
                        new BuscaProdutoVisao(
                                VendaUsuarioVisaoTop.this, // ou "this" conforme contexto
...
//        habilitarColunas();
    ...
    public VendaUsuarioVisaoTop( int cod_usuario, BDConexao conexao, String docPadraoPersonalizado ) throws SQLException
    {
...
//        }
    ...
    private void accao_codigo_manual_enter()
    {
        ...
        {
            ex.printStackTrace();
            Logger.getLogger( VendaUsuarioVisaoTop.class.getName() ).log( Level.SEVERE, null, ex );
...
//    }
    ...
    {
        new VendaUsuarioVisaoTop( 15, BDConexao.getInstancia() ).show( true );
    }
...
//refresh
    ...
    for (int i = 0; i < modelo.getRowCount(); i++) {
        try {
            ...
            Object valIdProduto = modelo.getValueAt(i, 0);
            Object valQtd = modelo.getValueAt(i, 4);
            Object valTaxa = modelo.getValueAt(i, 6);
            Object valTaxaR = modelo.getValueAt(i, 7);

            // Se o código do produto ou quantidade estiver vazio, ignora a linha
            if (valIdProduto == null || valIdProduto.toString().trim().isEmpty()) continue;
            if (valQtd == null || valQtd.toString().trim().isEmpty()) continue;

            // Converte para números
            int idProduto = Integer.parseInt(valIdProduto.toString().trim());
            int qtd = Integer.parseInt(valQtd.toString().trim());
            double taxa = (valTaxa != null && !valTaxa.toString().trim().isEmpty()) ? Double.parseDouble(valTaxa.toString().trim()) : 0;
            double taxa_r = (valTaxaR != null && !valTaxaR.toString().trim().isEmpty()) ? Double.parseDouble(valTaxaR.toString().trim()) : 0;

            // Calcula preço, desconto e totais
            double preco = getPreco(idProduto, qtd);
            double desconto = getDesconto_produto(preco, qtd);
            double sub_total_linha = (preco * qtd) - desconto;
            double sub_total_linha_com_iva = getValorComImposto(qtd, getTaxaImposto(idProduto), preco, desconto);
            double sub_total_linha_retencao = 0; // Se tiver cálculo, substitua aqui
            ...
            System.err.println("Linha " + i + " contém valor inválido, ignorando...");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//            precoDao.create(preco_novo_grosso);
    ...
    private void captura_tecla_tabela()
    {

        // Permitir capturar teclas na tabela
        table.addKeyListener( new KeyAdapter()
        {
            ...
            public void keyPressed( KeyEvent e )
            {
                ...
                if ( e.getKeyCode() == KeyEvent.VK_ENTER )
                {
                    ...
                    // Se estiver na primeira coluna (0), faz a pesquisa
                    if ( col == 0 )
                    {
                        e.consume(); // evita pular de célula
                        pesquisarProdutoNaTabela( row );
                    }
                }
            }
        } );
    ...
    private void pesquisarProdutoNaTabela( int row )
    {
        try
        {
            Object valor = table.getValueAt( row, 0 );

            if ( valor != null && !valor.toString().trim().isEmpty() )
            {
                int codigo = Integer.parseInt( valor.toString().trim() );

                TbProduto produto = (TbProduto) produtosController.findById( codigo );

                if ( produto != null )
                {
                    // Preenche outras colunas
                    table.setValueAt( produto.getDesignacao(), row, 1 ); // coluna descrição
                    table.setValueAt( produto.getCodUnidade().getDescricao(), row, 2 ); // unidade
                    table.setValueAt( produto.getPrecoVenda(), row, 3 ); // preço
                    table.setValueAt( 1, row, 4 ); // quantidade padrão

                    // Se quiser calcular o total automaticamente:
                    double total = produto.getPrecoVenda() * 1;
                    table.setValueAt( total, row, 5 );

                    // Mover o foco para próxima célula (ex: quantidade)
...
private void metodo_render_tabela() {

    DefaultTableModel modelo = new DefaultTableModel(
            new Object[]{"Código", "Descrição", "Preço", "Quantidade", "Total"}, 0
    ) {
        @Override
        public boolean isCellEditable(int row, int column) {
            return column == 0; // Apenas coluna "Código" editável
        }
    };
    ...
    // ⚙️ Define editor de texto para primeira coluna
    JTextField txtCodigo = new JTextField();
    txtCodigo.addActionListener(e -> {
        int row = table.getSelectedRow();
        int col = 0; // Coluna Código
        ...
        if (valor != null && !valor.toString().trim().isEmpty()) {
            try {
                ...
                TbProduto produto = (TbProduto) produtosController.findById(codigo);
                if (produto != null) {
                    table.setValueAt(produto.getDesignacao(), row, 1);
                    table.setValueAt(produto.getPrecoVenda(), row, 2);
                    table.setValueAt(1, row, 3);
                    table.setValueAt(produto.getPrecoVenda(), row, 4);

                    if (row == modelo.getRowCount() - 1) {
                        modelo.addRow(new Object[]{"", "", "", "", ""});
                    }

                    // Foca na coluna Quantidade
    ...
    table.getColumnModel().getColumn(0).setCellEditor(new DefaultCellEditor(txtCodigo));
}
