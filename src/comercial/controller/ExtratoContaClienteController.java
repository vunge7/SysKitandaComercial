...
import kitanda.controller.DocumentoController;
import kitanda.controller.NotasController;
import kitanda.controller.UsuarioController;
import kitanda.controller.VendaController;
...
public class ExtratoContaClienteController implements EntidadeFactory {
    ...
    public List<ExtratoContaCliente> listarTodos() {
        ...
        try {
            ...
            while (result.next()) {
                ...
                lista_extrato_conta_cliente.add(getExtratoContaCorrenteByResultSet(extrato_conta_cliente, result));
            }
    ...
    public Object findById(int codigo) {
        ...
        try {
            if (result.next()) {
                return getExtratoContaCorrenteByResultSet(extrato_conta_cliente, result);
            }
    ...
    public ExtratoContaCliente getExtratoContaCliente(long codigo) {
        ...
        try {
            if (result.next()) {
                extrato_conta_cliente = getExtratoContaCorrenteByResultSet(extrato_conta_cliente, result);
            }
    ...
    private ExtratoContaCliente getExtratoContaCorrenteByResultSet(ExtratoContaCliente extrato_conta_cliente, ResultSet result) {
        try {
...
//        System.out.println( "Cod Nota: " + listarAllNotaCreditoByCodFact.get( 0 ).getCodNota() );
        ...
        ExtratoContaClienteController extratoContaClienteController = new ExtratoContaClienteController(conexaoLocal);
        DocumentosController.start(conexaoLocal);
        ExtratoContaClienteController.gerarExtratoAutomatico(new Date(), new Date());
...
//                String data = new SimpleDateFormat( YYYYMMDD_HHMMSS ).format( data_hora );
                ...
                //if (do//cumento.equals("Factura")) {
                if (documento_object.getPkDocumento() == DVML.DOC_FACTURA_FT) {
                    ...
                    // } else if (documento.equals("Recibo Emitido") || documento.equals("Nota de crédito")) {
                } else if ((documento_object.getPkDocumento() == DVML.DOC_RECIBO_RC)
                        || (documento_object.getPkDocumento() == DVML.DOC_NOTA_CREDITO_NC)) {
    ...
    public static boolean gerarExtratoAutomatico(Date data_1, Date data_2) {
        ...
        List<TbVenda> lista_recibo = vendaController.listarTodosByData1AndData2AndTipoDocumento(data_1, data_2, DVML.DOC_RECIBO_RC);;
        List<TbVenda> lista_nota = vendaController.listarTodosByData1AndData2AndTipoDocumento(data_1, data_2, DVML.DOC_NOTA_CREDITO_NC);
            ...
            for (TbVenda factura : lista_factura) {
                ...
                if (!Objects.isNull(lista_recibo)) {
                    for (TbVenda recibo : lista_recibo) {
                        ...
                        //verifica se existe nota de credito deste recibo
//                        lista_nota = notasController.listarAllNotaCreditoByCodFact(recibo.getCodFact(), DVML.DOC_NOTA_CREDITO_NC);
                        lista_nota = vendaController.listarAllReciboByCodFact(recibo.getCodFact());
                        if (!Objects.isNull(lista_nota)) {
                            for (TbVenda notas : lista_nota) {
                 ...
                 */
                lista_nota = vendaController.listarAllNotasByCodFact(factura.getCodFact());
