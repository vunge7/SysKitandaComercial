...
public class CaixasController implements EntidadeFactory
{
    ...
    // 🔧 Construtor reforçado
    public CaixasController( BDConexao conexao )
    {
        // Se vier null, pega a instância singleton
        this.conexao = ( conexao != null ) ? conexao : BDConexao.getInstancia();

        // Garante que a conexão ativa seja recuperada (sem chamar conectar diretamente)
        if ( this.conexao.getConnectionAtiva() == null )
        {
            System.err.println( "[CaixasController] ⚠ Não foi possível obter conexão ativa!" );
        }
        else
        {
            System.out.println( "[CaixasController] ✅ Conexão ativa confirmada." );
        }
    }
    ...
    public boolean actualizar( Object object )
    {
        ...
        try ( PreparedStatement ps = conexao.getConnectionAtiva().prepareStatement( sql ) )
        {
            ...
            ps.setTimestamp( 2, caixa.getDataFecho() != null
                    ? new java.sql.Timestamp( caixa.getDataFecho().getTime() )
                    : null );
        ...
        {
            System.err.println( "Erro ao actualizar caixa: " + e.getMessage() );
            e.printStackTrace();
            return false;
        }
    }
    ...
    {
        String sql = "DELETE FROM caixa WHERE pk_caixa = " + codigo;
    ...
    {
        String sql = "SELECT * FROM caixa WHERE pk_caixa = " + codigo;
    ...
    // 🔍 Método seguro contra NullPointerException
    public boolean existeCaixas()
    {
        if ( conexao == null || conexao.getConnectionAtiva() == null )
        {
            System.err.println( "⚠️ Conexão inexistente em CaixasController.existeCaixas()" );
            return false;
        }

        String sql = "SELECT 1 FROM caixa LIMIT 1";
        try ( ResultSet rs = conexao.executeQuery( sql ) )
        {
            return rs.next();
        }
        catch ( SQLException e )
        {
            System.err.println( "Erro ao verificar caixas: " + e.getMessage() );
            return false;
        }
    }
    ...
    public boolean existeCaixas( int idUser )
    {
        if ( conexao == null || conexao.getConnectionAtiva() == null )
        {
            System.err.println( "⚠️ Conexão inexistente em CaixasController.existeCaixas(int)" );
            return false;
        }

        String sql = "SELECT 1 FROM caixa WHERE cod_usuario_abertura = " + idUser + " LIMIT 1";
        try ( ResultSet rs = conexao.executeQuery( sql ) )
        {
            return rs.next();
        }
        ...
        {
            System.err.println( "Erro ao verificar caixas por usuário: " + e.getMessage() );
            return false;
        }
    }
...
//                return rs.getInt( "maximo_id" );
    ...
    {
        Integer idCaixa = getLastCaixa( idUser );
        if ( idCaixa == null || idCaixa <= 0 )
        {
            return null;
        }
        return (Caixa) findById( idCaixa );
    }
