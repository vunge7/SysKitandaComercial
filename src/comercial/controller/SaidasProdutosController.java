...
//    }
    ...
    String sql = "INSERT INTO tb_saidas_produtos ("
            + "data_saida, fk_usuario, idArmazemFK, hora_saida, status_eliminado, "
            + "obs, nome_funcionario, documento, armazem_destino) "
            + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    try (PreparedStatement stmt = connection.prepareStatement(sql)) {
        ...
        // ✅ NOVO CAMPO
        stmt.setString(9, sp.getArmazemDestino());
    ...
    public void atualizar(TbSaidasProdutos sp) throws SQLException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            stmt.setString(8, sp.getDocumento());
            stmt.setString(9, sp.getArmazemDestino());
            stmt.setInt(10, sp.getPkSaidasProdutos());
            stmt.executeUpdate();
        }
    }
    ...
    private TbSaidasProdutos mapear(ResultSet rs) throws SQLException {
        ...
        sp.setArmazemDestino(rs.getString("armazem_destino"));
        return sp;
    }

    //Metodo de busca do armazem destino
    public Vector<String> getArmazemDestino() {
        Vector<String> lista = new Vector<>();

        String sql = "SELECT designacao FROM tb_parametros";
