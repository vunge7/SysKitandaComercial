...
//        String sql = "SELECT cod_nota FROM Notas WHERE cod_nota <  '" + cod_fact + "' ORDER BY cod_nota DESC LIMIT 1";
    ...
    public Notas findByCodFactReemprensaoNota( String codFact )
    {
        EntityManager em = getEntityManager();
        Query query = em.createQuery( "SELECT DISTINCT v  FROM notas  v WHERE V. = :COD_FACT AND V.statusEliminado = 'true' " );
        query.setParameter( "COD_FACT", codFact );
    ...
    public Notas findByCodFactReemprensao( String codFact )
    {
        EntityManager em = getEntityManager();
        Query query = em.createQuery( "SELECT DISTINCT v  FROM Notas  v WHERE V.codNota = :COD_FACT AND V.statusEliminado = 'true' " );
        query.setParameter( "COD_FACT", codFact );
