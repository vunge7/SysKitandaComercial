...
public class SaidasProdutosDao extends TbSaidasProdutosJpaController
{
    ...
    public List<TbSaidasProdutos> getAllSaidaByBetweenData( Date data_inicio, Date data_fim )
    {
        ...
        Query query = em.createNativeQuery( "SELECT * FROM tb_saidas_produtos "
                + "WHERE  DATE(data_saida) BETWEEN ? AND ? "
                + "AND status_eliminado = 'false'", TbSaidasProdutos.class );
...
//                .setParameter("data_inicio", MetodosUtil.getDataBanco( data_inicio ))
//                .setParameter("data_fim", MetodosUtil.getDataBanco( data_fim ));
//               
//        List<TbSaidasProdutos> result = query.getResultList();
//        em.close();
//       
...
////////////////////////////////////////////////////////////////////////////////////////////////////////    
    ...
    public TbSaidasProdutos findByCodFactReemprensao( String codFact )
    {
        EntityManager em = getEntityManager();
//        Query query = em.createQuery("SELECT DISTINCT v  FROM TbVenda  v WHERE V.codFact = :COD_FACT AND V.statusEliminado = 'false' ");
        Query query = em.createQuery( "SELECT DISTINCT s  FROM TbSaidasProdutos s WHERE s.documento = :COD_FACT " );
        query.setParameter( "COD_FACT", codFact );

        List<TbSaidasProdutos> documentos = query.getResultList();
        em.close();
